...
for entry in entries:
    ...
    if entry.operation == 'C':
        ...
        if not(lastComplete[0] == entry.time and lastComplete[1] == entry.block and lastComplete[2] == entry.length):
            lastComplete[0] = entry.time
            lastComplete[1] = entry.block
            lastComplete[2] = entry.length
            queueLength -= 1
print(queueLength)
